...
class NECAgent(nn.Module):
    ...
    def choose_action(
        ...
        print("NECAgent.choose_action() exploitation q_values:", q_values)
    ...
    def apply_memory_updates(
        ...
        Returns:
            Dictionary that contains insert and update counts for each action.
        """

        return self.update_strategy.apply(dnds=self.dnds, update_requests=update_requests)
    ...
    def check_dnd_key_gradients(self) -> None:
        ...
        for dnd in self.dnds:
            print(dnd.keys.requires_grad)
            print(dnd.keys.grad is None)

            if dnd.keys.grad is not None:
                print(dnd.keys.grad.norm())
